...
class ZWeights:
    ...
    ⚠️ 权重来源说明（重要）
    ----------------------------------------------------------------------
    以下默认权重不再是手工调参得到的初始值，而是经过 4 轮 CMA-ES
    （协方差矩阵自适应进化策略）在中证 A500 股票池上跑 30 代 × 16 个体
    优化得到的最优权重组合（**22 维**）。

    训练配置：
        算法          : CMA-ES (cma 4.4.4)
        训练区间      : 2025-03-01 ~ 2025-06-30（100 只抽样股票，81 个交易日）   ← v5 改用
        验证区间      : 2025-07-01 ~ 2025-10-30（500 只全量，82 个交易日）       ← v5 改用
        测试区间      : 2026-02-01 ~ 2026-06-30（500 只全量，89 个交易日）       ← v5 改用
        权重搜索范围  : init_value × [0.01, 10.0]  ← v5 进一步放宽
        训练脚本      : 06_RL/01_Ashares/01_Alpha/RL_Alpha_AShares.py
        训练产物      : 06_RL/01_Ashares/01_Alpha/outputs/best_weights_v5.json  ← v5 文件
    ...
    ⚠️ v5 与 v3 的对比
        ...
        可能原因：v5 训练/验证/测试时间窗更长（4/4/5 个月），市场环境差异大；
        测试集只 1 个数据点，统计意义有限。建议上线前再做 walk-forward 验证。
    ----------------------------------------------------------------------
